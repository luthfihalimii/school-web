...
<head>
    ...
    <title>{{ $article->title }} - SMKN 1 Surabaya</title>
...
<body class="bg-white text-gray-800">
    ...
    <main class="pt-24">
            ...
            <article class="max-w-4xl mx-auto">
                ...
                <header class="mb-8">
                    <div class="text-sm text-gray-500 mb-4">
                        ...
                        <a href="{{ route('news.index') }}" class="hover:text-red-600">Berita</a>
                    </div>
                    <span class="text-red-600 font-semibold text-sm">{{ strtoupper($article->category ?? 'Berita') }}</span>
                    <h1 class="text-3xl md:text-5xl font-extrabold text-gray-900 mt-2 leading-tight">{{ $article->title }}</h1>
                    <div class="flex items-center text-sm text-gray-500 mt-4">
                        <span>Oleh: Humas SMKN 1 Surabaya</span>
                ...
                <!-- Featured Image -->
                <figure class="mb-8">
                    <img class="w-full h-auto max-h-[500px] object-cover rounded-2xl shadow-lg" src="{{ $article->image_url }}" alt="{{ $article->title }}">
                    @if ($article->excerpt)
                        <figcaption class="text-center text-sm text-gray-500 mt-2">{{ $article->excerpt }}</figcaption>
                    @endif
                </figure>
                ...
                <div class="mt-12 pt-6 border-t">
                    <div class="flex items-center">
                        <span class="font-semibold mr-4">Bagikan:</span>
                        <div class="flex space-x-2">
                            <a href="#" class="w-10 h-10 flex items-center justify-center bg-gray-100 rounded-full hover:bg-red-100 text-gray-600 hover:text-red-600 transition-colors"><i data-lucide="facebook"></i></a>
                            <a href="#" class="w-10 h-10 flex items-center justify-center bg-gray-100 rounded-full hover:bg-red-100 text-gray-600 hover:text-red-600 transition-colors"><i data-lucide="twitter"></i></a>
                            <a href="#" class="w-10 h-10 flex items-center justify-center bg-gray-100 rounded-full hover:bg-red-100 text-gray-600 hover:text-red-600 transition-colors"><i data-lucide="linkedin"></i></a>
                             <a href="#" class="w-10 h-10 flex items-center justify-center bg-gray-100 rounded-full hover:bg-red-100 text-gray-600 hover:text-red-600 transition-colors"><i data-lucide="link-2"></i></a>
                        </div>
                    </div>
                </div>
            </article>
        ...
        <section class="py-16 bg-gray-50 mt-16">
            <div class="container mx-auto px-4 sm:px-6 lg:px-8">
                <h2 class="text-3xl font-bold text-center text-gray-900 mb-8">Berita Terkait Lainnya</h2>
                <div class="grid gap-8 lg:grid-cols-3 md:grid-cols-2 max-w-6xl mx-auto">
                    @forelse ($relatedArticles as $related)
                        <div class="bg-white rounded-2xl overflow-hidden shadow-lg hover:shadow-2xl transition-shadow duration-300 flex flex-col">
                            <img class="w-full h-56 object-cover" src="{{ $related->image_url }}" alt="{{ $related->title }}">
                            <div class="p-6 flex flex-col flex-grow">
                                <div class="flex-grow">
                                    @if ($related->category)
                                        <span class="text-sm font-semibold text-red-600">{{ $related->category }}</span>
                                    @endif
                                    <h3 class="mt-2 text-xl font-bold text-gray-900">{{ $related->title }}</h3>
                                </div>
                                <div class="mt-4 pt-4 border-t border-gray-100">
                                    <a href="{{ route('news.show', $related->slug) }}" class="inline-block text-red-600 font-semibold hover:text-red-800 text-sm">Baca Selengkapnya &rarr;</a>
                                </div>
                            </div>
                        </div>
                    @empty
                        <div class="lg:col-span-3 text-center text-gray-500">Belum ada berita terkait lainnya.</div>
                    @endforelse
                </div>
            </div>
        </section>
